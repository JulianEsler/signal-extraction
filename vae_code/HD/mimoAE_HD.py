...
class MIMONet(nn.Module):
    def __init__(self, sst_dim, precip_dim, latent_dim=1):
        ...
        self.shared_latent = nn.Linear(20, latent_dim)

        self.decoder_sst = nn.Sequential(
            nn.Linear(latent_dim, 10), nn.Tanh(), nn.Linear(10, 50), nn.Tanh(), nn.Linear(50, sst_dim)
        )
        self.decoder_precip = nn.Sequential(
            nn.Linear(latent_dim, 10), nn.Tanh(), nn.Linear(10, 50), nn.Tanh(), nn.Linear(50, precip_dim)
        )
...
def plot_latent(latent, title='Shared Latent Index from MIMO-AE'):
    """
    Plot the shared latent time series. Handles any number of latent dimensions.
    One subplot per latent dim. Returns the Figure.
    """
    latent = np.atleast_2d(np.array(latent).T).T   # (T, latent_dim)
    T, latent_dim = latent.shape
    t = np.arange(T)
    colors = plt.rcParams["axes.prop_cycle"].by_key()["color"]

    fig, axes = plt.subplots(latent_dim, 1, figsize=(15, max(4, 3 * latent_dim)), squeeze=False)
    fig.suptitle(title)
    for k in range(latent_dim):
        ax = axes[k, 0]
        ax.plot(t, latent[:, k], linewidth=1, color=colors[k % len(colors)],
                label=f"dim {k}" if latent_dim > 1 else "MIMO-AE Index")
        ax.set_ylabel(f"dim {k}" if latent_dim > 1 else "Latent Value")
...
def train_summary(results, file_name):
    ...
    # 1. Text summary
    axes[0].axis("off")
    text = (f"SST Loss: mean = {np.mean(results['test_sst_losses']):.10f}, std = {np.std(results['test_sst_losses']):.10f}\n"
            f"Precip Loss: mean = {np.mean(results['test_precip_losses']):.10f}, std = {np.std(results['test_precip_losses']):.10f}")
    axes[0].text(0.01, 0.6, text, fontsize=12)
    axes[0].set_title("Testing Loss Statistics")

    # 2. Training loss
    plot_loss_curve(
        loss1=results["precip_losses"],
        loss2=results["sst_losses"],
        label1="Precip Loss",
        label2="SST Loss",
    ...
    # 3. ND loss
    plot_nd_loss_curve(results["losses"], results["nd_sst_losses"], results["nd_precip_losses"],
                       "Training Nondimensional Losses", ax=axes[2])

    plt.tight_layout()
    pdf.savefig(fig)
    plt.close(fig)

    # FIGURE 2: latent time series
    fig_latent = plot_latent(results["latent_series"], "Shared Latent Index from MIMO-AE")
    pdf.savefig(fig_latent)
    plt.close(fig_latent)

    pdf.close()


def learn(sst_dat, precip_dat, norm, sst_var='sst', precip_var='tp', train_pct=0.8, batch=32, epochs=100, latent_dim=1, verbose=True):
    ...
    model = MIMONet(sst_flat.shape[1], precip_flat.shape[1], latent_dim)
    loss_fn = nn.MSELoss()
...
def extractr(model, sst_dat, svar, precip_dat, pvar, sst_scaler, precip_scaler, plot=True, save=False, save_path=None):
    ...
    if latent.ndim == 1:
        latent_series = xr.DataArray(latent, coords={"time": time}, dims=["time"], name="latent")
    else:
        latent_series = xr.DataArray(
            latent,
            coords={"time": time, "latent_dim": np.arange(latent.shape[1])},
            dims=["time", "latent_dim"],
            name="latent"
        )

    if plot:
        plot_latent(latent_series.values, title="Latent Series from New Dataset")
