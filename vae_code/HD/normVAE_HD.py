...
def plot_mu_logvar_z(mu_series, std_series, z_series, title_prefix="MIMOVAE"):
    ...
    Produces three figures (z, μ, logσ²), each with one subplot per latent dim.
    Returns (fig_z, fig_mu, fig_std).
    """
    # Normalize to shape (T, latent_dim)
    mu_series  = np.atleast_2d(np.array(mu_series).T).T
    std_series = np.atleast_2d(np.array(std_series).T).T
    z_series   = np.atleast_2d(np.array(z_series).T).T

    T, latent_dim = mu_series.shape
    t = np.arange(T)
    ...
    for k in range(latent_dim):
        ...
        ax.fill_between(t,
                        mu_series[:, k] - sigma_series[:, k],
                        mu_series[:, k] + sigma_series[:, k],
                        color=c, alpha=0.2, label="μ ± σ")
    ...
    # --- Figure 2: μ(t) ---
    fig_mu, axes_mu = plt.subplots(latent_dim, 1, figsize=(12, fig_h), squeeze=False)
    fig_mu.suptitle(f"{title_prefix}: Latent Mean (μ)")
    for k in range(latent_dim):
        axes_mu[k, 0].plot(t, mu_series[:, k], color=colors[k % len(colors)])
        axes_mu[k, 0].set_ylabel(dim_label(k) or "μ")
    axes_mu[-1, 0].set_xlabel("Time Index")
    plt.tight_layout()

    # --- Figure 3: logσ²(t) ---
    fig_std, axes_std = plt.subplots(latent_dim, 1, figsize=(12, fig_h), squeeze=False)
    fig_std.suptitle(f"{title_prefix}: Latent Log Variance (logσ²)")
    for k in range(latent_dim):
        axes_std[k, 0].plot(t, std_series[:, k], color=colors[k % len(colors)])
        axes_std[k, 0].set_ylabel(dim_label(k) or "logσ²")
    axes_std[-1, 0].set_xlabel("Time Index")
    plt.tight_layout()

    return fig_z, fig_mu, fig_std
...
def train_summary(results, file_name):
    ...
    # FIGURES 2-4: LATENT DIAGNOSTICS (z with μ±σ, μ, logσ²)
    fig_z, fig_mu, fig_std = plot_mu_logvar_z(mu, std, latent, title_prefix="MIMO-VAE")
    for f in (fig_z, fig_mu, fig_std):
        pdf.savefig(f)
        plt.close(f)
...
class MIMOVAE(nn.Module):
    def __init__(self, sst_dim, precip_dim, latent_dim=1):
        ...
        self.mu = nn.Linear(10, latent_dim)
        self.logvar = nn.Linear(10, latent_dim)
        ...
        latent_ds = xr.Dataset(
            {
                ...
                "mu": (["time", "latent_dim"], mu_series),
                "logvar": (["time", "latent_dim"], logvar_series),
            },
