"""
This version replaces the precipitation reconstruction loss with a Gamma negative log-likelihood (Gamma NLL) applied in raw physical units
(e.g., mm/month or m/month). Inputs to the VAE remain min–max normalized for stability, but precipitation outputs are inverse-transformed back to raw units
before the Gamma loss is evaluated. This change allows the model to capture the highly right-skewed, heavy-tailed distribution of monthly precipitation and
encourages the latent variable to represent extreme-precipitation behavior. All SST processing, scaling, and logging behavior remain unchanged.
"""
...
def learn(sst_dat, precip_dat, norm, sst_var='sst', precip_var='tp',
    ...
    def inverse_minmax_torch(scaler, x_scaled, device):
        """
        PyTorch-safe inverse of MinMaxScaler for computing Gamma NLL.
        Converts only the precipitation tensor back to raw units.
        DO NOT replace your existing inverse_transform() (numpy).
        """
        data_min  = torch.tensor(scaler.data_min_,  dtype=torch.float32, device=device)
    ...
    for epoch in range(epochs):
        for sst_batch, precip_batch in train_loader:
            ...
            # -----------------------
            # PRECIP LOSS (Gamma NLL)
            #   Must be applied in RAW units
            # -----------------------
            precip_true_raw  = inverse_minmax_torch(precip_scaler, precip_batch, device)
            precip_recon_raw = inverse_minmax_torch(precip_scaler, precip_recon, device)

            nd_p_loss = gamma_nll(precip_true_raw, precip_recon_raw, shape=0.5)
