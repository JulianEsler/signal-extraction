...
class MIMOVAE(nn.Module):
    def __init__(self, sst_dim, precip_dim):
        ...
        # === Precipitation decoder (unchanged) ===
    ...
    def forward(self, sst_x, precip_x):
        ...
        return sst_mu, sst_var, precip_hat, alpha, beta, z
...
def learn(sst_dat, precip_dat, norm, sst_var='sst', precip_var='tp', gshape=1, grate=1,
    ...
    """
    Train MIMOVAE with:
      - Gaussian NLL reconstruction loss for SST
      - Gamma NLL reconstruction loss for precipitation
      - KL divergence between Gamma posterior and Gamma prior in latent space

    Returns diagnostics, scalers, and latent series.
    """
    ...
    # -------------------------------
    # RECONSTRUCTION: Gamma NLL for precipitation
    # -------------------------------
    def gamma_nll(y_true, y_pred_mean, shape=0.5, eps=1e-6):
        ...
        shape_t = torch.tensor(shape, device=y_true.device)
        rate = shape_t / y_pred_mean  # β = k / μ
    ...
    for epoch in range(epochs):
        ...
        for sst_batch, precip_batch in train_loader:
            ...
            # Forward pass:
            # model should return: sst_mu, sst_var, precip_mu, alpha, beta, z
            ...
            # ------------------------------------------------------------------
            # Physical-space diagnostics (MSE after inverse scaling) – unchanged
            # ------------------------------------------------------------------
            # Uses inverse_transform(scaler, tensor) as in your original code
            sst_loss = np.mean(
