'''
Note that the HD suffix reflects that this code has been modified to have a higher dimension latent space.
'''
...
def plot_alpha_beta_z(alpha_series, beta_series, z_series, title_prefix="MIMOVAE"):
    """
    Plot alpha(t), beta(t), and z(t) for Gamma-latent VAE.
    Produces three figures (z with uncertainty, alpha, beta), each with one subplot per latent dim.
    Returns (fig_z, fig_alpha, fig_beta).
    """
    # Normalize to shape (T, latent_dim)
    alpha_series = np.atleast_2d(np.array(alpha_series).T).T
    beta_series  = np.atleast_2d(np.array(beta_series).T).T
    z_series     = np.atleast_2d(np.array(z_series).T).T

    T, latent_dim = z_series.shape
    t = np.arange(T)

    mu  = alpha_series / beta_series
    std = np.sqrt(alpha_series) / beta_series

    fig_h  = max(3 * latent_dim, 4)
    colors = plt.rcParams["axes.prop_cycle"].by_key()["color"]

    def dim_label(k):
        return f"dim {k}" if latent_dim > 1 else None

    # --- Figure 1: z(t) with Gamma mean ± std band ---
    fig_z, axes_z = plt.subplots(latent_dim, 1, figsize=(12, fig_h), squeeze=False)
    fig_z.suptitle(f"{title_prefix}: Latent Sample (z)")
    for k in range(latent_dim):
        ax = axes_z[k, 0]
        c  = colors[k % len(colors)]
        ax.plot(t, z_series[:, k], color=c, label="z")
        ax.fill_between(t, mu[:, k] - std[:, k], mu[:, k] + std[:, k],
                        color=c, alpha=0.2, label="Gamma mean ± std")
        ax.set_ylabel(dim_label(k) or "z")
        ax.legend(fontsize=8, loc="upper right")
    axes_z[-1, 0].set_xlabel("Time Index")
    plt.tight_layout()

    # --- Figure 2: alpha(t) ---
    fig_alpha, axes_alpha = plt.subplots(latent_dim, 1, figsize=(12, fig_h), squeeze=False)
    fig_alpha.suptitle(f"{title_prefix}: Latent Shape (alpha)")
    for k in range(latent_dim):
        axes_alpha[k, 0].plot(t, alpha_series[:, k], color=colors[k % len(colors)])
        axes_alpha[k, 0].set_ylabel(dim_label(k) or "alpha")
    axes_alpha[-1, 0].set_xlabel("Time Index")
    plt.tight_layout()

    # --- Figure 3: beta(t) ---
    fig_beta, axes_beta = plt.subplots(latent_dim, 1, figsize=(12, fig_h), squeeze=False)
    fig_beta.suptitle(f"{title_prefix}: Latent Rate (beta)")
    for k in range(latent_dim):
        axes_beta[k, 0].plot(t, beta_series[:, k], color=colors[k % len(colors)])
        axes_beta[k, 0].set_ylabel(dim_label(k) or "beta")
    axes_beta[-1, 0].set_xlabel("Time Index")
    plt.tight_layout()

    return fig_z, fig_alpha, fig_beta
...
def train_summary(results, file_name):
    ...
    # FIGURES 2-4: latent diagnostics (z with uncertainty, alpha, beta)
    fig_z, fig_alpha, fig_beta = plot_alpha_beta_z(alpha, beta, latent, title_prefix="MIMO-VAE (Gamma Latent)")
    for f in (fig_z, fig_alpha, fig_beta):
        pdf.savefig(f)
        plt.close(f)
...
class MIMOVAE(nn.Module):
    def __init__(self, sst_dim, precip_dim, latent_dim=1):
    ...
    def forward(self, sst_x, precip_x):
        ...
        h = torch.cat([sst_encoded, precip_encoded], dim=1)
...
def learn(sst_dat, precip_dat, norm, sst_var='sst', precip_var='tp', gshape=1, grate=2,
          train_pct=0.8, batch=32, epochs=100, latent_dim=1, verbose=True):
    ...
    # MODEL + OPTIMIZER
    # -------------------------------
    model = MIMOVAE(sst_flat.shape[1], precip_flat.shape[1], latent_dim)
    loss_fn = nn.MSELoss()
...
def extractr(model, sst_dat, svar, precip_dat, pvar, sst_scaler, precip_scaler,
    ...
    if z_series.ndim == 1:
        ...
        latent_ds = xr.Dataset(
            {
                "latent":  (["time", "latent_dim"], z_series),
                "alpha":   (["time", "latent_dim"], alpha_series),
                "beta":    (["time", "latent_dim"], beta_series),
                "lambda":  (["time", "latent_dim"], lambda_series),
            },
            coords={"time": time, "latent_dim": np.arange(z_series.shape[1])}
        )
