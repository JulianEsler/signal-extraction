...
class WaveletAnalyzer:
    ...
    def confidence_interval(
        ...
        for i in range(lp):
            p = (1 - (alpha**2)) / (1 + (alpha**2) - (2 * alpha * np.cos(2 * np.pi * freq[i])))
            df = 2 * np.sqrt(1 + ((na / (2.32 * ps[i])) ** 2))
            ci[i] = p * (chi2.ppf(conf, df) / df) * np.var(dat)
    ...
    def process_dataframe_columns(
        ...
        for column in cols:
            ...
            # write reconstruction back only where you actually computed it (non-NaN slice)
            df_out.loc[col_series.index, wave_col_name] = reconstruction
            
            # lagged feature (shift on the full df so it lines up with the master timeline)
            df_out[wave_lag_col_name] = df_out[wave_col_name].shift(1)
